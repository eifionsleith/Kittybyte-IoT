...
def check_and_feed(profiles):
    ...
    for profile in profiles:
        if profile.is_active_now():
            message = f"[FEEDING] User {profile.user_id}'s cat should be fed {profile.portion_grams}g now."
            events.append(message)
        else:
            message = f"[SKIP] No feeding needed now for {profile.user_id}."
            events.append(message)
    return events

# Sample profiles (can be replaced or extended)
if __name__ == "__main__":
    ...
    print(f"Checking feeding schedule at {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    results = check_and_feed(profiles)
