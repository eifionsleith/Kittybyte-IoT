...
def send_and_receive(port, baud, packet_id, frequency, duration_ms):
    ...
    if frequency <= 0 or frequency > 65535 or duration_ms <= 0 or duration_ms > 65535:
        print("Error: Frequency and Duration must be between 1 and 65535.")
        return
    ...
    try:
        ...
        # Wait for responses for a duration slightly longer than the task
        # to potentially catch both CMD_RECEIVED and TASK_COMPLETE
        responses_received = []
        overall_wait_time = (duration_ms / 1000.0) + 2.0 # Wait for duration + 2 extra seconds
        ...
        while time.time() < end_wait_time:
            ...
            if response:
                ...
                # If read_response_packet returns None, it means timeout for this attempt
                pass # Continue waiting until overall timeout

        if not responses_received:
            print(f"No valid response packets received within {overall_wait_time:.1f} seconds.")
